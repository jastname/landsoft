...
<script>
	...
	function openScoreEditPopup(studentId, grade, semester, scoreDivion, btn) {
		var scoreIds = [];
		var scores = [];
		var subjectIds = [];
		...
		var url = "score_edit.do?studentId=" + studentId + "&grade=" + grade
				+ "&semester=" + semester + "&scoreDivion=" + scoreDivion
...
<body>
	...
	<c:if test="${empty studentView}">
		...
		<div class="button-container">
			<a
				href="student_view_input.do?studentId=${param.studentId}&name=${param.name}">등록</a>
		</div>
	</c:if>
	...
	<c:if test="${not empty studentScore}">
		<table class="dataTable" style="margin: auto;">
			...
			<c:forEach var="list" items="${studentScore}" varStatus="status">
				<c:if
					test="${rowCount < 4 && (list.semester != previousSemester || list.scoreDivion != previousScoreDivion)}">
					<tr>
                        ...
                        <c:if test="${list.semester == score.semester && list.scoreDivion == score.scoreDivion && subject.subjectId == score.subjectId}">
                            data-score-id="${score.scoresId}"
								...
								<c:set var="subjectScore" value="" /> <c:forEach var="score"
									items="${studentScore}">
									<c:if
										test="${list.semester == score.semester && list.scoreDivion == score.scoreDivion && subject.subjectId == score.subjectId}">
						...
						<td><c:choose>
								<c:when test="${empty list.totalScore}">
									<button type="button"
										onclick="openScoreInputPopup('${studentView.studentId}', '${param.grade}')">등록</button>
								</c:when>
								<c:otherwise>
									<button type="button"
										onclick="openScoreEditPopup('${studentView.studentId}', '${param.grade}','${list.semester}','${list.scoreDivion}', this)">수정</button>
								</c:otherwise>
							</c:choose></td>
					</tr>
        ...
        <!-- 부족한 행 추가 -->
        <c:set var="emptyRows" value="${4 - fn:length(studentScore)}" />
            ...
            <c:forEach var="i" begin="0" end="${emptyRows - 1}">
                <tr>
                    ...
                    <td><button type="button"
                            onclick="openScoreInputPopup('${studentView.studentId}', '${param.grade}')">등록</button></td>
                </tr>
            </c:forEach>
        </c:if>
    </table>
...
<c:if test="${empty studentScore}">
	...
	<div class="button-container">
		<a href="student_list.do">목록</a> <a
			href="student_delete.do?studentId=${studentView.studentId}"
